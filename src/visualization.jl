function D3Trees.D3Tree(p::POMCPOWRAVEPlanner; title="POMCPOWRAVE Tree", kwargs...)
    @warn("""
         D3Tree(planner::POMCPOWRAVEPlanner) is deprecated and may be removed in the future. Instead, please use
    ...
    if p.tree == nothing
        error("POMCPOWRAVEPlanner has not constructed a tree yet, run `action(planner, belief)` first to construct the tree.")
    end
    return D3Tree(p.tree; title=title, kwargs...)
end

function D3Trees.D3Tree(t::POMCPOWRAVETree; title="POMCPOWRAVE Tree", kwargs...)
...
Base.show(io::IO, mime::MIME"text/html", t::POMCPOWRAVETree) = show(io, mime, D3Tree(t))
Base.show(io::IO, mime::MIME"text/plain", t::POMCPOWRAVETree) = show(io, mime, D3Tree(t))
